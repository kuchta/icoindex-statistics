...
export function txMockService(startBlockNumber: number, blocks: Block[], host: string, port: number, listening?: (address: string, port: number) => void) {
	...
	blocks = R.map((block) => R.assoc('timestamp', String(Date.parse(block.timestamp) / 1000), block), blocks);
	// let blocks = R.map(R.over(R.lensProp('timestamp'), R.compose(String, Date.parse)), testData.fixtures);

	app.all('/ethereum', getEthereumMock(blocks, startBlockNumber));
	app.all('/etherscan', getEtherscanMock(blocks));
...
function getEthereumMock(blocks: Block[], currentBlockNumber: number) {
	const lastBlockNumber = Number(R.reduce(R.max, 0, R.pluck('number', blocks)));
	return function mockEthereum(req: Request, res: Response) {
		...
		if (request.method === 'eth_blockNumber') {
			...
			const block = R.find(R.propEq('number', request.params[0]), blocks) || null;
			if (block) {
...
function getEtherscanMock(blocks: Block[]) {
	return function mockEtherscan(req: Request, res: Response) {
		let result;
